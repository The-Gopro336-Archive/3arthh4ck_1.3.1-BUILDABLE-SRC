...
extends ModuleListener<Surround, PacketEvent.Receive<SPacketSpawnObject>> {
    ...
    private void placeAsync(SPacketSpawnObject packet, EntityPlayer player) {
        ...
        AxisAlignedBB bb = new EntityEnderCrystal(ListenerSpawnObject.mc.world, packet.getX(), packet.getY(), packet.getZ()).getEntityBoundingBox();
        ...
        for (BlockPos pos : surrounding) {
            EnumFacing facing;
            if (!bb.intersects(new AxisAlignedBB(pos)) || !ListenerSpawnObject.mc.world.getBlockState(pos).func_185904_a().isReplaceable() || (facing = BlockUtil.getFacing(pos, ListenerSpawnObject.mc.world)) == null) continue;
        ...
        Locks.acquire(Locks.PLACE_SWITCH_LOCK, () -> {
            ...
            for (Map.Entry entry : toPlace.entrySet()) {
                float[] helpingRotations = RotationUtil.getRotations((BlockPos)entry.getKey(), (EnumFacing)entry.getValue(), player);
                RayTraceResult result = RayTraceUtil.getRayTraceResultWithEntity(helpingRotations[0], helpingRotations[1], player);
                if (((Surround)this.module).rotate.getValue() == Rotate.Packet) {
                    ListenerSpawnObject.mc.player.connection.sendPacket(new CPacketPlayer.Rotation(helpingRotations[0], helpingRotations[1], ListenerSpawnObject.mc.player.onGround));
                }
                float[] f = RayTraceUtil.hitVecToPlaceVec((BlockPos)entry.getKey(), result.hitVec);
                ListenerSpawnObject.mc.player.connection.sendPacket(new CPacketPlayerTryUseItemOnBlock((BlockPos)entry.getKey(), (EnumFacing)entry.getValue(), InventoryUtil.getHand(slot), f[0], f[1], f[2]));
