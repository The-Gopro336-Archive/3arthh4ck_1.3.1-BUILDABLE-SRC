    ...
    public void invoke(PacketEvent.Receive<SPacketPlayerPosLook> event) {
        if (((NoRotate)this.module).noForceLook.getValue().booleanValue() && !event.isCancelled()) {
            ...
            if (((NoRotate)this.module).async.getValue().booleanValue()) {
                PacketUtil.handlePosLook((SPacketPlayerPosLook)event.getPacket(), ListenerPosLook.mc.player, true);
            } else {
                mc.addScheduledTask(() -> PacketUtil.handlePosLook((SPacketPlayerPosLook)event.getPacket(), ListenerPosLook.mc.player, true));
            }
        }
    }
}
