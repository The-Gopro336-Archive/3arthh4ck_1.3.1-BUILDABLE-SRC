            ...
            while ((double)z <= (double)this.mZ + this.range) {
                ...
                while ((double)y < (double)this.mY + this.range) {
                    block21: {
                        ...
                        IBlockState state;
                        block22: {
                            if (this.dsq(x, y, z) > rSquare || this.dsq((float)x + 0.5f, y + 1, (float)z + 0.5f) >= bSquare && !RayTraceUtil.canBeSeen(new Vec3d((float)x + 0.5f, (double)y + 2.7, (float)z + 0.5f), (Entity)RotationUtil.getRotationPlayer())) break block21;
                            mPos.setPos(x, y, z);
                            state = AutoMineCalc.mc.world.getBlockState(mPos);
                            boolean bl = isObbyState = state.getBlock() == Blocks.OBSIDIAN || state.getBlock() == Blocks.BEDROCK;
                            if (!this.obby && !isObbyState || !isObbyState && !state.func_185904_a().isReplaceable() && !MineUtil.canBreak(state, mPos)) break block21;
                            mPos.setY(y + 1);
                            upState = AutoMineCalc.mc.world.getBlockState(mPos);
                            if (upState.getBlock() != Blocks.AIR && !MineUtil.canBreak(upState, mPos) || upState.getBlock() == Blocks.OBSIDIAN && !this.mineObby || upState.getBlock() != Blocks.AIR && this.dsq(x, y + 1, z) > rSquare) break block21;
                            upUpState = null;
                            if (this.newVer) break block22;
                            mPos.setY(y + 2);
                            upUpState = AutoMineCalc.mc.world.getBlockState(mPos);
                            if (upUpState.getBlock() != Blocks.AIR && !MineUtil.canBreak(upUpState, mPos) || upUpState.getBlock() == Blocks.OBSIDIAN && !this.mineObby || upUpState.getBlock() != Blocks.AIR && this.dsq(x, y + 2, z) > rSquare) break block21;
                        }
                        boolean bad = false;
                        for (Entity entity : this.entities) {
                            if (entity.preventEntitySpawning && !isObbyState) {
                                mPos.setY(y);
                                if (BBUtil.intersects(entity.getEntityBoundingBox(), mPos)) {
                                    bad = true;
                                    break;
                                }
                            }
                            ...
                            mPos.setY(y + 2);
                            if (!BBUtil.intersects(entity.getEntityBoundingBox(), mPos)) continue;
                            bad = true;
                            break;
                        }
                        if (!bad) {
                            ...
                            mPos.setY(y);
                            helper.addBlockState(mPos, Blocks.OBSIDIAN.getDefaultState());
                            ...
                            float self = DamageUtil.calculate((float)x + 0.5f, y + 1, (float)z + 0.5f, RotationUtil.getRotationPlayer().getEntityBoundingBox(), RotationUtil.getRotationPlayer(), helper, true);
                            if (this.suicide || !(self > this.maxSelf)) {
                                ...
                                if (this.target == null) {
                                    for (EntityPlayer player : this.players) {
                                        float f;
                                        float d;
                                        if (player.getDistanceSq(x, y, z) > 144.0 || !((d = DamageUtil.calculate((float)x + 0.5f, y + 1, (float)z + 0.5f, player.getEntityBoundingBox(), player, helper, true)) > damage)) continue;
                                        damage = d;
                                        if (!(f > this.minDamage)) continue;
                                        break;
                                    }
                                } else {
                                    damage = DamageUtil.calculate((float)x + 0.5f, y + 1, (float)z + 0.5f, this.target.getEntityBoundingBox(), this.target, helper, true);
                                }
