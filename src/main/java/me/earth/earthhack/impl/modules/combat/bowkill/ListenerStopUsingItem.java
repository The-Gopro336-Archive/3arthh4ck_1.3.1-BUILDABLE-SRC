                ...
                for (int i = 0; i < ((BowKiller)this.module).runs.getValue() + ((BowKiller)this.module).buffer.getValue(); ++i) {
                    if (i != 0 && i % ((BowKiller)this.module).interval.getValue() == 0) {
                        ...
                        for (int j = 0; j < ((BowKiller)this.module).teleports.getValue(); ++j) {
                            ListenerStopUsingItem.mc.player.connection.sendPacket(new CPacketConfirmTeleport(++id));
                        }
                    }
                    ...
                    if (((BowKiller)this.module).rotate.getValue().booleanValue()) {
                        ...
                        if (((BowKiller)this.module).target != null) {
                            float[] rotations = ((BowKiller)this.module).rotationSmoother.getRotations(RotationUtil.getRotationPlayer(), ((BowKiller)this.module).target, ((BowKiller)this.module).height.getValue(), ((BowKiller)this.module).soft.getValue().floatValue());
