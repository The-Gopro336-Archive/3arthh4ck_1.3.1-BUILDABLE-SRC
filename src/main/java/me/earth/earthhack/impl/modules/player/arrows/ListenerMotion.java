...
extends ModuleListener<Arrows, MotionUpdateEvent> {
    ...
    public void invoke(MotionUpdateEvent event) {
        ...
        if (event.getStage() == Stage.PRE) {
            ...
            if (player.motionX != 0.0 || player.motionZ != 0.0) {
                Vec3d vec3d = player.getPositionVector().addVector(player.motionX, player.motionY + (double)player.getEyeHeight(), player.motionZ);
    ...
    private boolean blocked() {
        ...
        return ListenerMotion.mc.world.getBlockState(pos.up()).func_185904_a().blocksMovement() || ListenerMotion.mc.world.getBlockState(pos.up(2)).func_185904_a().blocksMovement();
    }
}
