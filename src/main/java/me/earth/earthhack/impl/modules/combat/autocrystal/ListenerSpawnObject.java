...
extends ModuleListener<AutoCrystal, PacketEvent.Receive<SPacketSpawnObject>> {
    ...
    private void onEvent(PacketEvent.Receive<SPacketSpawnObject> event) {
        ...
        CrystalTimeStamp stamp = ((AutoCrystal)this.module).placed.get(pos);
        ...
        if (!((AutoCrystal)this.module).alwaysCalc.getValue().booleanValue() && stamp != null && stamp.isValid() && (stamp.getDamage() > ((AutoCrystal)this.module).slowBreakDamage.getValue().floatValue() || ((AutoCrystal)this.module).breakTimer.passed(((AutoCrystal)this.module).slowBreakDelay.getValue().intValue()) || pos.down().equals(((AutoCrystal)this.module).antiTotemHelper.getTargetPos()))) {
            float damage = this.checkPos(entity);
            if (damage <= -1000.0f) {
                MutableWrapper<Boolean> a = new MutableWrapper<Boolean>(false);
                ((AutoCrystal)this.module).rotation = ((AutoCrystal)this.module).rotationHelper.forBreaking(entity, a);
                event.addPostEvent(() -> {
                    Entity e;
            ...
            float self = this.checkPos(entity);
            if (self < 0.0f) {
                return;
            }
            boolean slow = true;
            ...
            for (EntityPlayer player : players) {
                if (player == null || EntityUtil.isDead(player) || player.getDistanceSq(x, y, z) > 144.0) continue;
                if (Managers.FRIENDS.contains(player)) {
                    if (!((AutoCrystal)this.module).antiFriendPop.getValue().shouldCalc(AntiFriendPop.Break) || !(((AutoCrystal)this.module).damageHelper.getDamage(x, y, z, player) > EntityUtil.getHealth(player) - 0.5f)) continue;
                    attack = false;
                    break;
                }
                float dmg = ((AutoCrystal)this.module).damageHelper.getDamage(x, y, z, player);
    ...
    private void attack(SPacketSpawnObject packet, PacketEvent.Receive<?> event, EntityEnderCrystal entityIn, boolean slow) {
        ...
        Runnable runnable = () -> {
            ...
            ListenerSpawnObject.mc.player.connection.sendPacket(p);
    ...
    private float getSelfDamage(Entity entity) {
        ...
        if (damage > EntityUtil.getHealth(ListenerSpawnObject.mc.player) - 1.0f || damage > DMG.getValue().floatValue()) {
            Managers.SAFETY.setSafe(false);
        }
        return damage > ((AutoCrystal)this.module).maxSelfBreak.getValue().floatValue() || damage > EntityUtil.getHealth(ListenerSpawnObject.mc.player) - 1.0f && ((AutoCrystal)this.module).suicide.getValue() == false ? -1.0f : damage;
    }
}
