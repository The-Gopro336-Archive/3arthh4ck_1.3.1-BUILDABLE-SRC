...
extends ModuleListener<BowKiller, MotionUpdateEvent> {
    ...
    public void invoke(MotionUpdateEvent event) {
        ...
        if (event.getStage() == Stage.PRE) {
            ...
            if (((BowKiller)this.module).rotate.getValue().booleanValue() && ListenerMotion.mc.player.getActiveItemStack().getItem() == Items.BOW && ListenerMotion.mc.gameSettings.keyBindUseItem.isKeyDown() && ((BowKiller)this.module).blockUnder) {
                ...
                if (((BowKiller)this.module).target != null && (rotations = ((BowKiller)this.module).rotationSmoother.getRotations(RotationUtil.getRotationPlayer(), ((BowKiller)this.module).target, ((BowKiller)this.module).height.getValue(), ((BowKiller)this.module).soft.getValue().floatValue())) != null) {
            ...
            if (ListenerMotion.mc.player.getActiveItemStack().getItem() == Items.BOW && ListenerMotion.mc.player.isHandActive() && !((BowKiller)this.module).blockUnder && (newSlot = this.findBlockInHotbar()) != -1) {
                ...
                this.placeBlock(PositionUtil.getPosition(RotationUtil.getRotationPlayer()).down(1), event);
    ...
    private int findBlockInHotbar() {
        for (int i = 0; i < 9; ++i) {
            ...
            if (stack == ItemStack.EMPTY || !(stack.getItem() instanceof ItemBlock) || !((block = ((ItemBlock)((Object)stack.getItem())).getBlock()) instanceof BlockObsidian)) continue;
            return i;
        }
        return -1;
    }
    ...
    private boolean isBlockUnder() {
        return !(ListenerMotion.mc.world.getBlockState(PositionUtil.getPosition(RotationUtil.getRotationPlayer()).down(1)).getBlock() instanceof BlockAir);
    }
}
