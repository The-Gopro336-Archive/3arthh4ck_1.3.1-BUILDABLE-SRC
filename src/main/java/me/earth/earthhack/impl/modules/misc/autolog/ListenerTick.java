...
extends ModuleListener<AutoLog, TickEvent> {
    ...
    public void invoke(TickEvent event) {
        if (ListenerTick.mc.world != null && ListenerTick.mc.player != null) {
            float health;
            float f = health = ((AutoLog)this.module).absorption.getValue() != false ? EntityUtil.getHealth(ListenerTick.mc.player) : ListenerTick.mc.player.getHealth();
            if (health <= ((AutoLog)this.module).health.getValue().floatValue()) {
                ...
                if ((((AutoLog)this.module).enemy.getValue().floatValue() == 100.0f || player != null && player.getDistanceSq(ListenerTick.mc.player) <= (double)MathUtil.square(((AutoLog)this.module).enemy.getValue().floatValue())) && (totems = InventoryUtil.getCount(Items.TOTEM_OF_UNDYING)) <= ((AutoLog)this.module).totems.getValue()) {
