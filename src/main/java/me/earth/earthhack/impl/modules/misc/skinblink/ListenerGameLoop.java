    ...
    public void invoke(GameLoopEvent event) {
        if (((SkinBlink)this.module).timer.passed(((SkinBlink)this.module).delay.getValue().intValue())) {
            for (EnumPlayerModelParts parts : EnumPlayerModelParts.values()) {
                ListenerGameLoop.mc.gameSettings.setModelPartEnabled(parts, ((SkinBlink)this.module).random.getValue().booleanValue() ? Math.random() < 0.5 : !ListenerGameLoop.mc.gameSettings.getModelParts().contains(parts));
            }
