...
public class FlatUIUtils {
    ...
    public static void paintArrow(Graphics2D g, int x, int y, int width, int height, int direction, boolean chevron, int arrowSize, float xOffset, float yOffset) {
        ...
        float ox = (float)(width - (aw + extra)) / 2.0f + UIScale.scale(xOffset);
        float oy = (float)(height - (ah + extra)) / 2.0f + UIScale.scale(yOffset);
        int ax = x + (direction == 7 ? -Math.round(-ox) : Math.round(ox));
        int ay = y + (direction == 1 ? -Math.round(-oy) : Math.round(oy));
        g.translate(ax, ay);
